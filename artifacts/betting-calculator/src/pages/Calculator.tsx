...
// ─── Types ────────────────────────────────────────────────────────────────────
type MenuItem = "kalkulator" | "laporan" | "result" | "statistik";
...
export default function Calculator({ theme, toggleTheme }: { theme: "dark"|"light"; toggleTheme: () => void }) {
  ...
  return (
    <div className={`${isDark ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-900"} min-h-screen transition-all duration-500`}>
          ...
          <div className="flex items-center gap-1.5">
            ...
            {isGithubSetup && (
              <button onClick={() => { saveToGithub(); toast.info("Menyimpan ke GitHub..."); }}
                ...
                className={`p-2 rounded-xl transition-all ${isDark ? "bg-white/10 hover:bg-white/15 text-white" : "bg-slate-100 hover:bg-slate-200 text-slate-700"} disabled:opacity-50`}>
            ...
            <button onClick={() => { setShowProfile(true); setShowNotif(false); setShowGuide(false); setShowGithub(false); }}
              className={`p-2 rounded-xl transition-all ${isDark ? "bg-white/10 hover:bg-white/15 text-white" : "bg-slate-100 hover:bg-slate-200 text-slate-700"}`}>
      ...
      {!isGithubSetup && (
        <div className="max-w-7xl mx-auto px-3 pt-3 relative z-10">
          <button onClick={() => setShowGithub(true)}
            className="w-full px-4 py-3 rounded-2xl bg-gradient-to-r from-purple-600/80 to-blue-600/80 border border-purple-500/40 text-white text-sm font-bold flex items-center justify-center gap-2 hover:scale-[1.01] transition-all">
            <Github className="w-4 h-4" />
            Setup GitHub Database — Data tersimpan permanen & bisa diakses dari perangkat manapun →
          </button>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-3 md:px-5 py-4 relative z-10 space-y-4">
        {/* ── Nav tabs ── */}
        <div className={`${mobileMenuOpen ? "flex" : "hidden"} md:flex flex-col md:flex-row gap-2 flex-wrap`}>
          <div className="flex gap-1.5 flex-wrap flex-1">
            {(["kalkulator","laporan","result","statistik"] as MenuItem[]).map(m => (
              <button key={m} onClick={() => { setMenu(m); setMobileMenuOpen(false); }} className={tabBtn(menu === m)}>
                {m==="kalkulator" && <Target className="w-3.5 h-3.5"/>}
                {m==="laporan"    && <FileText className="w-3.5 h-3.5"/>}
                {m==="result"     && <Award className="w-3.5 h-3.5"/>}
                {m==="statistik"  && <BarChart2 className="w-3.5 h-3.5"/>}
                {m.charAt(0).toUpperCase() + m.slice(1)}
              </button>
            ))}
          </div>
          <div className="flex gap-1.5 flex-wrap">
            <button onClick={copyStrategi}     className="px-3 py-2.5 rounded-xl bg-green-600 text-white font-bold text-xs hover:scale-105 active:scale-95 transition-all flex items-center gap-1"><Copy className="w-3 h-3"/>Copy</button>
            <button onClick={exportCSV}        className="px-3 py-2.5 rounded-xl bg-purple-600 text-white font-bold text-xs hover:scale-105 active:scale-95 transition-all flex items-center gap-1"><Download className="w-3 h-3"/>CSV</button>
            <button onClick={exportJSON}       className="px-3 py-2.5 rounded-xl bg-cyan-700 text-white font-bold text-xs hover:scale-105 active:scale-95 transition-all flex items-center gap-1"><Download className="w-3 h-3"/>Backup</button>
            <button onClick={importJSON}       className="px-3 py-2.5 rounded-xl bg-cyan-600 text-white font-bold text-xs hover:scale-105 active:scale-95 transition-all flex items-center gap-1"><Upload className="w-3 h-3"/>Import</button>
            <button onClick={resetSesi}        className="px-3 py-2.5 rounded-xl bg-orange-600 text-white font-bold text-xs hover:scale-105 active:scale-95 transition-all flex items-center gap-1"><RotateCcw className="w-3 h-3"/>Sesi Baru</button>
            <button onClick={hapusHistori}     className="px-3 py-2.5 rounded-xl bg-red-600 text-white font-bold text-xs hover:scale-105 active:scale-95 transition-all flex items-center gap-1"><Trash2 className="w-3 h-3"/>Hapus</button>
          </div>
        </div>
                    ...
                    <div className="flex flex-wrap justify-between gap-3">
                      ...
                      {!disabled && (
                        <div className="flex items-center gap-2">
                          <button onClick={() => simpanMenang(item)} className="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-green-500 to-emerald-600 text-white font-black shadow-lg hover:scale-105 active:scale-95 transition-all text-sm">✓ MENANG</button>
                          <button onClick={() => simpanKalah(item)} className="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-red-500 to-rose-600 text-white font-black shadow-lg hover:scale-105 active:scale-95 transition-all text-sm">✗ KALAH</button>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        {menu === "laporan" && (
          <div className="animate-slide-up space-y-4">
            ...
            <div className={`${cardCls} p-5`}>
              <h2 className="text-lg font-black mb-3">Histori Sesi</h2>
              <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                {histori.length === 0 && <div className="text-center p-8 opacity-50">Belum ada histori</div>}
                {histori.map((h, i) => (
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-40 md:hidden border-t ${isDark ? "bg-black/90 border-white/10" : "bg-white/95 border-slate-200"} backdrop-blur-2xl safe-area-pb pb-safe`}>
        <div className="flex">
          {([
            { id:"kalkulator", label:"Kalkulator", icon:<Target className="w-5 h-5"/> },
            { id:"laporan",    label:"Laporan",    icon:<FileText className="w-5 h-5"/> },
            { id:"result",     label:"Result",     icon:<Award className="w-5 h-5"/> },
            { id:"statistik",  label:"Statistik",  icon:<BarChart2 className="w-5 h-5"/> },
          ] as { id: MenuItem; label: string; icon: React.ReactNode }[]).map(item => (
            <button key={item.id} onClick={() => setMenu(item.id)}
              className={`flex-1 flex flex-col items-center justify-center py-2 gap-0.5 text-[10px] font-bold transition-all ${
                menu === item.id
                  ? "text-blue-400"
              ...
              {item.icon}
              {item.label}
              {menu === item.id && <div className="absolute bottom-0 h-0.5 w-8 bg-blue-400 rounded-full"/>}
            </button>
          ))}
        </div>
      </div>
